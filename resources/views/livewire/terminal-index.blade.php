<div class="max-w-6xl mx-auto">
    ...
    <x-jet-button class="btn btn-primary btn-sm" wire:click="showTerminalModal">登録</x-jet-button>
    </div>
    <div class="m-2 p-2">
        ...
        <table class="w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="p-1 text-gray-500 text-left border" style="width:850px">端末名</th>
            ...
            <tbody class="bg-white divide-y divide-gray-200 border">
                ...
                <tr>
                    <td class="p-1 whitespace-nowrap border">{{ $terminal->id }}</td>
                    <td class="p-1 whitespace-nowrap border">{{ $terminal->terminal_nm }}</td>
                    @if(nl2br($terminal->delete_flg)==1)
                    <td class="p-1 whitespace-nowrap border">削除中</td>
                    @else
                    <td class="p-1 whitespace-nowrap border"></td>
                    @endif
                    <td class="p-1 text-right text-sm border">
                        <x-jet-button class="bg-green-600" wire:click="showEditTerminalModal({{ $terminal->id }})">編集</x-jet-button>
                        <x-jet-button class="bg-red-400" wire:click="deleteTerminalinfo({{ $terminal->id }})">削除</x-jet-button>
                    </td>
                </tr>
                @endforeach
            </tbody>
        </table>
        <div class="m-2 p-2">{{ $terminals->links() }}</div>
    </div>
            ...
            <section>
                <table>
                    ...
                    <th class="border table-primary" style="width:150px;height:25px;">
                        <x-jet-label for="terminal_nm" value="端末名" />
                    </th>
                <td class="border">
                    ...
                    @error('terminal_nm') <span class="error text-red-400">{{ $message }}</span> @enderror
                    </td>
                    </tr>
                </table>
                <table>
